...
print("\n=== Test 데이터(2023년 하반기~) 모델 예측 확률 분포 ===")
bins = [0.0, 0.35, 0.40, 0.45, 0.48, 0.50, 0.52, 0.55, 0.60, 0.65, 1.0]
hist, bin_edges = np.histogram(probs, bins=bins)

for i in range(len(hist)):
    # 막대 그래프 문자열 생성
    bar_len = int((hist[i] / len(probs)) * 50)
    bar = "#" * bar_len
    
    pct = (hist[i] / len(probs)) * 100
    print(f"{bins[i]:.2f} ~ {bins[i+1]:.2f} : {hist[i]:>6} 개 ({pct:>5.2f}%) | {bar}")

print(f"\n총 예측 횟수(15분봉 개수): {len(probs)} 개")
print(f"최소 확률: {probs.min():.4f} / 최대 확률: {probs.max():.4f}")
print(f"평균 확률: {probs.mean():.4f}")
